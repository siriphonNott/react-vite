...
import { fetchTodos, updateToto, removeTodo } from './todosSlice'
  ...
  const TodoItem = ({ id, title, completed }) => {
    ...
    return (
      <div className={`hadow-md p-3 mb-3 rounded-lg flex items-center justify-between cursor-pointer shadow-md ${bgItem}`}>
        ...
        <div className="flex">
          { completed ? 
              <svg onClick={() => onUpdateToto(id, completed)} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-gray-500 hover:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              : <svg onClick={() => onUpdateToto(id, completed)} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-gray-500 hover:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
          }
          <svg onClick={() => onRemoveTodo(id)} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-500 hover:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        </div>
      </div>
    )
  }

  const onUpdateToto = (id, completed) => {
    try {
      dispatch(updateToto({ id }))
      !completed ? alertSuccess('The task has been done.') : alertWarning('The task has been pending.')
